...
这条规则的意义是：时间推进属于全局 replay 语义，视角切换属于身体观察语义，二者不能混在一个控制区里。
...
- 左侧：实验参数与状态
- 右上：脑图
- 中间：共享 replay timeline
- 右下：身体观察器

在 replay 模式下做三点强化：

1. 右下身体区升级成真正 inspector viewport（观察视口）
2. `timeline（时间轴）` 升级成共享一级组件，并放在右侧脑图和身体区之间
...
### 9.1 Timeline Strip Structure

共享 `timeline` 采用 `three-part strip（三段式控制条）`，而不是单滑条或厚控制卡。
...
- 明确区分“控制”“时间位置”“状态”
- 避免把共享时间轴退化成一条普通播放器滑条
- 保持右下身体区专注于视口和相机控制，而不是承担全局播放职责
...
- 共享 timeline 位于右侧脑图和身体区之间，并作为唯一全局回放主控条
